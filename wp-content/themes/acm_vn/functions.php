<?php 
    ...
    function my_single_templates($single_template) {
        ...
        if($post->post_type == 'product'){
            $single_template = dirname( __FILE__ ) . '/single_product.php';
        }else{
            $single_template = dirname( __FILE__ ) . '/single_news.php';
        }
    ...
    function my_page_templates($page_template) {
        ...
        if($post->ID == '0'){
            ...
            if($check_template == 'product'){
                $page_template = dirname( __FILE__ ) . '/page_product.php';
            }else{
                $page_template = dirname( __FILE__ ) . '/attribute_product.php';
            }
        }else{
            if($post->post_name == 'gio-hang'){
                $page_template = dirname( __FILE__ ) . '/cart.php';
            }else{
                $page_template = dirname( __FILE__ ) . '/page_news.php';
            }
        }
    ...
    function get_sale_percent($regular_price,$sale_price){
        if($regular_price > '0'){
            $sale_amount = (int)$regular_price - (int)$sale_price;
            $percent = round(($sale_amount / (int)$regular_price) * 100);
            if($percent == '100'){
                return 'HOT';
            }else{
                return '-'.$percent.'%';
    ...
    // Evergreen: Gutenberg Editor -> Classic Editor
    //chuyển trình soạn thảo văn bản về phiên bản cũ
    add_filter('use_block_editor_for_post', '__return_false');

    
    // /*------- add style css vao visual editor soan thao trong post --------*/
    // //cái này hơi quê -- thi thoảng xài

    // add_filter( 'tiny_mce_before_init', function ($mce_init) {

    //     $content_css = get_stylesheet_directory_uri() .'/6custom-editor-style.css';
    ...
    //     add_editor_style( '6custom-editor-style.css' );
    // }
